...
namespace Dynamo.Models
{
    ...
    public class DynamoModel : ModelBase
    {
        ...
        /// </summary>
        /// <param name="command">This parameter contains information that node
        /// creation requires. See CreateNodeCommand for details.</param>
        /// <param name="xmlNode">This parameter is provided when the resulting
        /// node needs to load itself from a known source. A typical scenario is
        /// a Paste operation, where the resulting node must be loaded from the
        /// XmlNode generated by the copied node.</param>
        ...
        internal NodeModel CreateNodeInternal(CreateNodeCommand command, XmlNode xmlNode)
        {
            ...
            NodeModel node = CreateNodeInstance(command.NodeName);
            if (node == null)
            {

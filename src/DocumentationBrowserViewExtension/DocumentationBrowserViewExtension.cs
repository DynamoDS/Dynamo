    ...
    public class DocumentationBrowserViewExtension : ViewExtensionBase, IViewExtension, ILogSource, ILayoutSpecSource
    {
        ...
        private void GroupInsertedGraph(List<AnnotationViewModel> existingGroups, string graphName)
        {
            ...
            if (annotation != null)
            {
                ...
                if(styleItem == null)
                {
                    //If no GroupStyle is found matching the specific criteria we will use the first one available
                    styleItem = annotationViewModel.GroupStyleList.OfType<GroupStyleItem>().FirstOrDefault();
                }
                if (styleItem != null)
                {

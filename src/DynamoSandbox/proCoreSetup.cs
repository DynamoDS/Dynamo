    ...
    class DynamoCoreSetup
    {
        ...
        public void RunApplication(Application app)
        {
            try
            {
                ...
                using (var authProvider = new AdWebServicesAuthProvider())
                {
                    ...
                    var startconfig = new DynamoViewModel.StartConfiguration
                    {
                        ...
                        BrandingResourceProvider = new DynamoProBrandingResourceProvider(),
                        DynamoModel = model,
                        ShowLogin = true
                    };
...
#if DYNAMO_LICENSING
                DynamoProLicensing.Licensing.OnCrash();
#endif
            }
        ...
        {
            var config = UpdateManagerConfiguration.GetSettings(new StudioLookUp());
        ...
        internal void SaveWorkspaceAlertAndClose()
        {
            if(viewModel != null)
            {
                // Give user a chance to save (but does not allow cancellation)

...
This file should be NUnit2 format and it is the result of a set of running test, there is an example below:
Note: NUnit 2 is the minimun required version.
...
```
Where C:\\DSCoreNodesTests.dll is the DLL that contains the set of test, DSCoreNodesTests.ColorTests is the name space of the test and C:\\TestResult-Color_Tests.xml is the path and the name of the file where the result will be. This file is the result of running/explore(only NUnit3) tests so the Transformation Tool will not be the one that creates the XML file in NUnit2 format.

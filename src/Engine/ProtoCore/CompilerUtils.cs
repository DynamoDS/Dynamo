    ...
    public class CompilerUtils
    {
        ...
        private static IEnumerable<AST.Node> ParseUserCodeCore(Core core, string expression)
        {
            ...
            foreach (var node in nodes)
            {
                ...
                {
                    core.BuildStatus.LogSemanticError(Resource.logSemanticErrorModifierBlocksAreNotSupported);
                }
                else if (n is ProtoCore.AST.AssociativeAST.ImportNode)
                {
                    core.BuildStatus.LogSemanticError(Resource.logSemanticErrorImportStatementsAreNotSupported);
                }

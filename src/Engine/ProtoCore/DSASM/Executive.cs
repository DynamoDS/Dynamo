...
namespace ProtoCore.DSASM
{
    public class Executive : IExecutive
    {
        ...
        /// <returns></returns>
        public StackValue Bounce(int exeblock, int entry, StackFrame stackFrame, int locals = 0, bool fepRun = false, Executive exec = null)
        {
            if (stackFrame != null)
            {
        ...
        private void RestoreFromCall()
        {
            ...
            if (IsExplicitCall)
            {
                // This condition should only be reached in the following case:
        ...
        protected virtual void CALLR_Handler(Instruction instruction)
        {
            ...
            {
                RX = CallrForMemberFunction(blockIndex, classIndex, functionIndex, ref explicitCall);
            }
            else if (!runtimeCore.Options.IsDeltaExecution)
            {
                RX = Callr(blockIndex, functionIndex, classIndex, ref explicitCall, isDynamicCall);
            }
            else
            {
                //

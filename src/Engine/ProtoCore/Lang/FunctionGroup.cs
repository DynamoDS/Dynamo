...
namespace ProtoCore
{
	...
	public class FunctionGroup
	{
            ...
            List<StackValue> formalParams, List<ReplicationInstruction> replicationInstructions, StackFrame stackFrame, Core core)
        {
            ...
            bool callOnInstance = thisptr.IsPointer && thisptr.opdata != Constants.kInvalidIndex;
                                  
            foreach (FunctionEndPoint fep in FunctionEndPoints)
            {
                if (callOnInstance)
                {
                    if (context.IsReplicating == !fep.procedureNode.isAutoGeneratedThisProc)
                    {
                        continue;
                    }
                }
                ...
                if (fep.DoesTypeDeepMatch(reducedParamSVs, core))
                {
                    if (thisptr.IsPointer &&
                        thisptr.opdata == Constants.kInvalidIndex && 
                        fep.procedureNode != null && 
                        !fep.procedureNode.isConstructor && 
                        !fep.procedureNode.isStatic && 
                        fep.procedureNode.classScope != Constants.kGlobalScope)
                    {
                        continue;
                    }

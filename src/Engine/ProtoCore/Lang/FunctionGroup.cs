	...
	public class FunctionGroup
	{
            ...
            MSILRuntimeCore runtimeCore)
        {
            ...
            foreach (CLRFunctionEndPoint fep in feps)
            {
                ...
                // TODO_MSIL: figure out how replication (`isReplicating`) affects these conditions
                bool isReplicating = false;
                if (isInstance && isReplicating != proc.IsAutoGeneratedThisProc)
                {
                    continue;
                }
                else if (isGlobal && !proc.IsConstructor && !proc.IsStatic && proc.ClassID != Constants.kGlobalScope)
        ...
        internal static bool CheckInvalidArrayCoercion(CLRFunctionEndPoint fep, CLRStackValue[] reducedSVs, MSILRuntimeCore runtimeCore, bool allowArrayPromotion)
        {
            ...
            for (int i = 0; i < reducedSVs.Length; i++)
            {
                ...
                var arrayTypes = ArrayUtils.GetTypeStatisticsForArray(reducedSVs[i], runtimeCore);

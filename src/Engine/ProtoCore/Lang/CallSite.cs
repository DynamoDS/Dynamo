    ...
    public class CallSite
    {
            ...
            FunctionEndPoint finalFunctionEndPoint = null)
        {
            ...
            if (ri.Zipped)
            {
                ...
                // We can use parallel.For for FFIFunctionEndPoint type and replication count is down to 1.
                if (replicationInstructions.Count == 1 && finalFunctionEndPoint is FFIFunctionEndPoint)
                {
                    //Preintialize FFI Endpoint Point outside of the Parallel loop
                    FFIFunctionEndPoint ffep = (FFIFunctionEndPoint)finalFunctionEndPoint;
                    ffep.Init(runtimeCore);

                    //Onetime adjustment to stackFrame ThisPtr.  This code exist within FFIFunctionEndPoint it is not thread safe there.
                    //Would need lock for stackFrame within that implemenation but in practice this codepath only is required to hit once per loop.
                    if (stackFrame != null)
                    {
                        //moved here from ExecWithZeroRI...  Only needs to be called once per look.
                    ...
                    Parallel.For(0, retSize, i =>
                    {
                        ...
                        //if(stackFrame != null)
                        //{
                        //    newStackFrame = new StackFrame(0);
                        //    newStackFrame.ThisPtr = stackFrame.ThisPtr;
                        //}
                        ...
                        //if(stackFrame != null)
                        //{
                        //    stackFrame.ThisPtr = newStackFrame.ThisPtr;
                        //}
            ...
            else
            {
                ...
                //Now iterate over each of these option
                //We can use parallel.For for FFIFunctionEndPoint type and replication count is down to 1.
                if (replicationInstructions.Count == 1 && finalFunctionEndPoint is FFIFunctionEndPoint)
                {
                    //Preintialize FFI Endpoint Point outside of the Parallel loop

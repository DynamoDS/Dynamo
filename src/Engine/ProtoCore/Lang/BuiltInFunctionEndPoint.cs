...
namespace ProtoCore.Lang
{
    public class BuiltInFunctionEndPoint : FunctionEndPoint
    {
        ...
        private StackValue DotMethod(StackValue lhs, StackFrame stackFrame, DSASM.Executive runtime, Context context)
        {
            ...
            {
                core.RuntimeStatus.LogWarning(WarningID.kDereferencingNonPointer, Resource.kDeferencingNonPointer);
    ...
    internal class FileIOBuiltIns
    {
        ...
        internal static StackValue LoadCSVWithMode(StackValue fn, StackValue trans, ProtoCore.DSASM.Interpreter runtime)
        {
            ...
            {
                // Type mismatch.
                runtime.runtime.Core.RuntimeStatus.LogWarning(RuntimeData.WarningID.kInvalidArguments, ProtoCore.Resource.kInvalidArguments);
            ...
            {
                string message = String.Format(Resource.kFileNotFound, path);
                                                   ...
                                                   ProtoCore.Core core)
        {
            if (!svStart.IsNumeric || !svEnd.IsNumeric)
            {
                core.RuntimeStatus.LogWarning(
                    ...
                    Resource.kInvalidArgumentsInRangeExpression);
                return StackValue.Null;
            }
            ...
            if (hasAmountOp)
            {
                if (!svEnd.IsNumeric)
                {
                    core.RuntimeStatus.LogWarning(
                        WarningID.kInvalidArguments, 
                        Resource.kInvalidAmountInRangeExpression);
                    return StackValue.Null;
                }
                else if (!hasStep)
                {
                    core.RuntimeStatus.LogWarning(
                        WarningID.kInvalidArguments, 
                        Resource.kNoStepSizeInAmountRangeExpression);
                    return StackValue.Null;
                }
            }
    ...
    internal class ArrayUtilsForBuiltIns
    {
        ...
        internal static bool Exists(StackValue sv, ProtoCore.DSASM.Interpreter runtime, Predicate<StackValue> pred)
        {
            ...
            {
                // Type mismatch.
                runtime.runtime.Core.RuntimeStatus.LogWarning(RuntimeData.WarningID.kInvalidArguments, Resource.kInvalidArguments);
                return false;
            }
        ...
        internal static StackValue Remove(StackValue sv1, StackValue sv2, ProtoCore.DSASM.Interpreter runtime)
        {
            ...
            {
                // Type mismatch.
                runtime.runtime.Core.RuntimeStatus.LogWarning(RuntimeData.WarningID.kIndexOutOfRange, Resource.kIndexOutOfRange);
                return DSASM.StackValue.Null;
            }
            List<StackValue> svList = new List<StackValue>();

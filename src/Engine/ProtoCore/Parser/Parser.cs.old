...
langblock.codeblock.Language == ProtoCore.Language.NotSpecified) {
	...
	void Associative_DictionaryExpression(out ProtoCore.AST.AssociativeAST.AssociativeNode node) {
		Expect(46);
		var dictExpr = new ProtoCore.AST.AssociativeAST.DictionaryNode(); 
		NodeUtils.SetNodeStartLocation(dictExpr, t); 
		if (la.kind == 4) {
			Get();
			dictExpr.Keys.Add(t.val); 
			Expect(48);
			Associative_Expression(out node);
			dictExpr.Values.Add(node); 
			while (la.kind == 52) {
				Get();
				Expect(4);
				dictExpr.Keys.Add(t.val); 
		...
		Expect(47);
		NodeUtils.SetNodeEndLocation(dictExpr, t); 
		node = dictExpr.ToFunctionCall(); 
	}

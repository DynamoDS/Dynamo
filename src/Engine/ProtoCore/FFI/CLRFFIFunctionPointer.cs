    ...
    class CLRFFIFunctionPointer : FFIFunctionPointer
    {
        ...
        public override object Execute(ProtoCore.Runtime.Context c, Interpreter dsi)
        {
            ...
            if (!ReflectionInfo.IsStatic)
            {
                ...
                {
                    string message = String.Format(ProtoCore.StringConstant.kFFIFailedToObtainThisObject, ReflectionInfo.DeclaringType.Name, ReflectionInfo.Name);
            ...
            for (int i = 0; i < mArgTypes.Length; ++i)
            {
                ...
                {
                    string message = String.Format(ProtoCore.StringConstant.kFFIFailedToObtainObject, paraminfos[i].ParameterType.Name, ReflectionInfo.DeclaringType.Name, ReflectionInfo.Name);
                    dsi.LogWarning(ProtoCore.RuntimeData.WarningID.kAccessViolation, message);
                    return null;
                }
            }

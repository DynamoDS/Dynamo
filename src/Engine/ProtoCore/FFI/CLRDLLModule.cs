...
namespace ProtoFFI
{
    ...
    public class CLRModuleType
    {
        ...
        public bool ReflectionContext
        {
            get { return Module.Assembly?.ReflectionOnly ?? Module.Module.Assembly.ReflectionOnly; }
        }
        ...
        private ClassDeclNode ParseSystemType(Type type, string alias)
        {
            ...
            foreach (var f in fields)
            {
                ...
                classnode.Variables.Add(variable);
                FunctionDefinitionNode func = ParseFieldAccessor(f);
                if (null != func && !ReflectionContext)
                    RegisterFunctionPointer(func.Name, f, null, func.ReturnType);
            }
        ...
        private ProtoCore.AST.AssociativeAST.AssociativeNode ParseProperty(PropertyInfo p)
        {
            ...
            var attribs = ReflectionContext ? 
                p.AttributesFromReflectionContext() : 
        ...
        private static bool SupressesImport(MemberInfo member, Dictionary<MethodInfo, Attribute[]> getterAttributes)
        {
            ...
            var reflectionContext = member.Module.Assembly.ReflectionOnly;
            object[] atts = null;
            if (reflectionContext)
            {
                atts = member.AttributesFromReflectionContext();
            }
            else
            {
        ...
        private ArgumentSignatureNode ParseArgumentSignature(MethodBase method)
        {
            ...
            foreach (var parameter in parameters)
            {
                ...
                var ffiAttribute = new FFIParamAttributes(parameter, ReflectionContext);
                ...
                if (parameter.IsOptional)
                {
                    ...
                    var defaultValue = ReflectionContext ? parameter.RawDefaultValue : parameter.DefaultValue;
            ...
            argumentSignature.IsVarArg = parameters.Any() && 
                (ReflectionContext ? 
                    parameters.Last().AttributesFromReflectionContext().Any(x => x is ParamArrayAttribute) : 
    ...
    public class CLRDLLModule : DLLModule
    {
        ...
        public override void ScanModule()
        {
            ...
            foreach (var type in types)
            {
                ...
                if (!type.IsGenericType && type.IsPublic && !CLRModuleType.SupressesImport(type))
                {
                    try
                    {
        ...
        private Type[] GetReflectionTypes(string typeName)
        {
            ...
            if (string.IsNullOrEmpty(typeName) || string.IsNullOrWhiteSpace(typeName))
            {
                if (Module is null)
                    ...
                    types = Module.Assembly.DefinedTypes.ToList();
            }
    ...
    public class FFIClassAttributes : ClassAttributes
    {
        ...
        public FFIClassAttributes(Type type)
        {
            ...
            attributes = type.Assembly.ReflectionOnly ?
                type.AttributesFromReflectionContext() :
            ...
            foreach (var attr in attributes)
            {
                ...
                //var a = Activator.CreateInstance(type, args);
    ...
    public class FFIMethodAttributes : ProtoCore.AST.AssociativeAST.MethodAttributes
    {
        ...
        public FFIMethodAttributes(MethodInfo method, Dictionary<MethodInfo, Attribute[]> getterAttributes)
        {
            ...
            else
            {
                if (reflectionContext)
                {
                    attributes = method.AttributesFromReflectionContext();
                }
                else
                {
    ...
    public class FFIParamAttributes: ProtoCore.AST.AssociativeAST.ExternalAttributes
    {
        ...
        {
            var attributes = reflectionOnly ? parameter.AttributesFromReflectionContext() : parameter.GetCustomAttributes(false);
            foreach (var attr in attributes)
            {

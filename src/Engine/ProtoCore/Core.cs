    ...
    public class Core
    {
        ...
        public StackValue Bounce(int exeblock, int entry, Context context, StackFrame stackFrame, int locals = 0, EventSink sink = null)
        {
            if (stackFrame != null)
            {
                ...
                int ci = (int)stackFrame.GetAt(StackFrame.AbsoluteIndex.kClass).opdata;
                int fi = (int)stackFrame.GetAt(StackFrame.AbsoluteIndex.kFunction).opdata;
                int returnAddr = (int)stackFrame.GetAt(StackFrame.AbsoluteIndex.kReturnAddress).opdata;
                int blockDecl = (int)stackFrame.GetAt(StackFrame.AbsoluteIndex.kFunctionBlock).opdata;
                int blockCaller = (int)stackFrame.GetAt(StackFrame.AbsoluteIndex.kFunctionCallerBlock).opdata;
                StackFrameType callerFrameType = (StackFrameType)stackFrame.GetAt(StackFrame.AbsoluteIndex.kCallerStackFrameType).opdata;
                StackFrameType frameType = (StackFrameType)stackFrame.GetAt(StackFrame.AbsoluteIndex.kStackFrameType).opdata;
                Validity.Assert(frameType == StackFrameType.kTypeLanguage);
                
                int depth = (int)stackFrame.GetAt(StackFrame.AbsoluteIndex.kStackFrameDepth).opdata;
                int framePointer = (int)stackFrame.GetAt(StackFrame.AbsoluteIndex.kFramePointer).opdata;

...
namespace ProtoCore.DSASM
{
    ...
    public class ProcedureTable
    {
        ...
        public int Append(ProcedureNode node)
        {
            ...
            {
                var procNode = Procedures[index];
                if (!procNode.IsActive)
                {
                    Procedures[index] = node;
                    node.ID = index;
                    return node.ID;
                }
            }
        ...
        public int IndexOfExact(string name, List<ProtoCore.Type> args, bool isAutoGeneratedThisProc)
        {
            ...
            {
                if (name == Procedures[n].Name && 
                    args.Count == Procedures[n].ArgumentTypes.Count &&
                    Procedures[n].IsAutoGeneratedThisProc == isAutoGeneratedThisProc)
                {
                    bool isMatch = true;
                    for (int i = 0; i < args.Count; ++i)
                    {
                        if ((Procedures[n].ArgumentTypes[i].UID != args[i].UID) ||
                            (Procedures[n].ArgumentTypes[i].IsIndexable != args[i].IsIndexable) ||
                            (Procedures[n].ArgumentTypes[i].rank != args[i].rank))
                        {
                            isMatch = false;
                            break;
                        }

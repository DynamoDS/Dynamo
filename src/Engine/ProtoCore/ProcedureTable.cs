...
namespace ProtoCore.DSASM
{
   ...
   //     public List<ProtoCore.Type> argTypeList;    // List of arguments required by the procedure
    ...
    public class ProcedureTable
    {
        ...
        {
            int index = IndexOfExact(node.name, node.Arguments, node.isAutoGeneratedThisProc);
        ...
        public IEnumerable<ProcedureNode> GetFunctionsBy(string name, int argumentNumber)
        {
            ...
            {
                int defaultArgumentNumber = p.Arguments.Count(X => X.DefaultExpression != null);
                int parameterNumber = p.Arguments.Count;
                return p.name.Equals(name) &&
                       (argumentNumber <= parameterNumber) &&
                       (argumentNumber - parameterNumber <= defaultArgumentNumber);
            }).OrderBy(p => p.Arguments.Count - argumentNumber);
        }
        ...
        public ProcedureNode GetFirst(string name, int argCount)
        {
            ...
            {
                if (name == procList[n].name && argCount == procList[n].Arguments.Count)
                {
        ...
        public int IndexOf(string name, List<ProtoCore.Type> argTypeList, bool isStaticOrConstructor = false)
        {
            ...
            for (int n = 0; n < procList.Count; ++n)
            {
                ...
                var argNum = proc.Arguments.Count;
                var paramNum = argTypeList.Count;
                int defaultArgNum = proc.Arguments.Count(X => X.DefaultExpression != null);
        ...
        public int IndexOfExact(string name, List<ArgumentInfo> args, bool isAutoGeneratedThisProc)
        {
            ...
            {
                if (name == procList[n].name && 
                    args.Count == procList[n].Arguments.Count &&
                    procList[n].isAutoGeneratedThisProc == isAutoGeneratedThisProc)
                {
                    bool isMatch = true;
                    for (int i = 0; i < args.Count; ++i)
                    {
                        if ((procList[n].Arguments[i].type.UID != args[i].type.UID) ||
                            (procList[n].Arguments[i].type.IsIndexable != args[i].type.IsIndexable) ||
                            (procList[n].Arguments[i].type.rank != args[i].type.rank))
                        {
                            isMatch = false;
                            break;
                        }

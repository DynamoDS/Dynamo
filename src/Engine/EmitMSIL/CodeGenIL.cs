...
namespace EmitMSIL
{
    public class CodeGenIL : IDisposable
    {
        ...
        private Type EmitCoercionCode(AssociativeNode arg, Type argType, Type param)
        {
            ...
            if (typeof(IList).IsAssignableFrom(argType) && typeof(IEnumerable<double>).IsAssignableFrom(param))
            {
                return EmitIListCoercion<double>(arg);
            }
            if (typeof(IList).IsAssignableFrom(argType) && typeof(IEnumerable<int>).IsAssignableFrom(param))
            {
                return EmitIListCoercion<int>(arg);
            }
            if (typeof(IList).IsAssignableFrom(argType) && typeof(IEnumerable<long>).IsAssignableFrom(param))
            {
        ...
        private IEnumerable<ProtoCore.CLRFunctionEndPoint> FunctionLookup(IList args)
        {
            ...
            foreach (var funcEnd in fg.FunctionEndPoints)
            {
                ...
                bool isInstanceMethod = !procNode.IsStatic &&
                    !procNode.IsConstructor && !CoreUtils.IsInternalMethod(methodName);

                // Instance methods have the "this" pointer as the first argument in "args"
                bool matchingArgCount = isInstanceMethod ?
                    (funcEnd.FormalParams.Length + 1) == args.Count :
                    funcEnd.FormalParams.Length == args.Count;

                if (!matchingArgCount) {
                    continue;
                }

                var procedureArgs = funcEnd.FormalParams.ToList();
                ...
                else if (funcEnd is FFIFunctionEndPoint ffiFep)
                {
                    ...
                    FFIFunctionPointer functionPointer = handler.GetFunctionPointer(ffiFep.activation.ModuleName, className,
                            ffiFep.activation.FunctionName, procedureArgs, ffiFep.activation.ReturnType);
                ...
                Validity.Assert(parameterInfos.Length == procedureArgs.Count, "Expected argument counts to match");
                var fepParams = new List<ProtoCore.CLRFunctionEndPoint.ParamInfo>();

                if (isInstanceMethod || procNode.IsAutoGeneratedThisProc)
                {

    ...
    public class CodeGen : ProtoCore.CodeGen
    {
            ...
            )
        {
            ...
            if (!allocateForBaseVar && !isProperty && core.ClassTable.IndexOf(ident) != ProtoCore.DSASM.Constants.kInvalidIndex)
                buildStatus.LogSemanticError(ident + Resource.className, null, line, col, graphNode);
                                ...
                                ref ProtoCore.Type inferedType)
        {
                ...
                if (toResolveMethodOnClass)
                {
                    ...
                    if (CoreUtils.TryGetPropertyName(procName, out property))
                    {
                        ...
                        if (procCallNode == null)
                        {
                            ...
                            {
                                string message = String.Format(StringConstants.kCallingNonStaticProperty,
                                                               className,
                                                               property);
                    ...
                    else
                    {
                        ...
                        if (!isStaticCall && !isConstructor)
                        {
                            if (subPass == ProtoCore.Compiler.Associative.SubCompilePass.kNone)
                            {
                                string message = String.Format(StringConstants.kStaticMethodNotFound,
                                                               className,
                                                               procName);
                ...
                else if (hasAllocated && symbolnode.datatype.UID != (int)PrimitiveType.kTypeVar)
                {
                    ...
                    if (null != procCallNode)
                    {
                        if (procCallNode.isConstructor)
                        {
                            if (subPass == ProtoCore.Compiler.Associative.SubCompilePass.kNone)
                            {
                                string message = String.Format(StringConstants.KCallingConstructorOnInstance,
                                                               procName);
            ...
            if ((procCallNode == null) && (procName != Constants.kFunctionPointerCall) && (parentNode == null))
            {
                if (globalClassIndex != Constants.kInvalidIndex)
                {
                    ...
                    if (memProcNode != null)
                    {
                        ...
                        if (!isAccessible)
                        {
                            string message = String.Format(StringConstants.kMethodIsInaccessible, procName);
            ...
            else
            {
                    ...
                    (globalClassIndex == inferedType.UID))
                {
                    ...
                    {
                        string message = String.Format(StringConstants.kCallingConstructorInConstructor, procName);
        ...
        */
                ...
                if (symbolnode == null)
                {
                    ...
                    if (isAllocated)
                    {
                        ...
                        if (localProcedure != null && localProcedure.isStatic)
                        {
                            ...
                            {
                                message = String.Format(StringConstants.kUsingNonStaticMemberInStaticContext, t.Value);
                            }
                        }
                       ...
                       else
                       {
                               ...
                               subPass == ProtoCore.Compiler.Associative.SubCompilePass.kNone)
                           {
                               buildStatus.LogWarning(WarningID.kInvalidRangeExpression,
                                                      StringConstants.kRangeExpressionWithNonIntegerStepNumber,
                                                      core.CurrentDSFileName,
                                                      stepNode.line,
                                                      stepNode.col, 
                                                      graphNode);
        ...
        private void EmitLanguageBlockNode(AssociativeNode node, ref ProtoCore.Type inferedType, ProtoCore.AssociativeGraph.GraphNode graphNode, ProtoCore.Compiler.Associative.SubCompilePass subPass = ProtoCore.Compiler.Associative.SubCompilePass.kNone)
        {
            if (IsParsingGlobal() || IsParsingGlobalFunctionBody() || IsParsingMemberFunctionBody() )
            {
                ...
                if (ProtoCore.Language.kAssociative == langblock.codeblock.language && !isTopBlock)
                {
                    ...
                    buildStatus.LogSyntaxError(Resource.associativeLanguageBlockIsDeclaredInside, core.CurrentDSFileName, langblock.line, langblock.col);
                }
            ...
            GraphNode graphNode = null)
        {
            ...
            if (ProtoCore.Compiler.Associative.CompilePass.kClassName == compilePass)
            {
                ...
                {
                    buildStatus.LogSemanticError(Resource.classCannotBeDefinedInsideALanguageBlock, core.CurrentDSFileName, classDecl.line, classDecl.col);
                }
        ...
        private void EmitConstructorDefinitionNode(AssociativeNode node, ref ProtoCore.Type inferedType, ProtoCore.Compiler.Associative.SubCompilePass subPass = ProtoCore.Compiler.Associative.SubCompilePass.kNone, GraphNode gNode = null)
        {
            ...
            if (IsParsingMemberFunctionSig())
            {
                ...
                {
                    string message = String.Format(StringConstants.kMethodAlreadyDefined, localProcedure.name);
        ...
        private void EmitBinaryExpressionNode(AssociativeNode node, ref ProtoCore.Type inferedType, bool isBooleanOp = false, ProtoCore.AssociativeGraph.GraphNode graphNode = null, ProtoCore.Compiler.Associative.SubCompilePass subPass = ProtoCore.Compiler.Associative.SubCompilePass.kNone, bool isTempExpression = false)
        {
            ...
            */
            ...
            else //(ProtoCore.DSASM.Operator.assign != b.Optr)
            {
                ...
                {
                    buildStatus.LogSemanticError(Resource.functionPointerIsNotAllowedAtBinaryExpression, core.CurrentDSFileName, bnode.LeftNode.line, bnode.LeftNode.col);
                }
            ...
            if (bnode.Optr != ProtoCore.DSASM.Operator.assign)
            {
                ...
                {
                    buildStatus.LogSemanticError(Resource.functionPointerIsNotAllowedAtBinaryExpression, core.CurrentDSFileName, bnode.RightNode.line, bnode.RightNode.col);
                }
            ...
            if (bnode.LeftNode is IdentifierNode)
            {
                ...
                else
                {
                    ...
                    {
                        string message = String.Format(StringConstants.kPropertyIsInaccessible, t.Name);

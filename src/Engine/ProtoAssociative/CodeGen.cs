            ...
            GraphNode graphNode = null)
        {
            ...
            else if (ProtoCore.CompilerDefinitions.Associative.CompilePass.kClassMemFuncSig == compilePass)
            {
                ...
                foreach (AssociativeNode funcdecl in classDecl.funclist)
                {
                    ...
                    if (funcDef != null)
                    {
                        string procName = funcDef.Name;
                        bool isFunctionExcluded = procName.Equals(Constants.kStaticPropertiesInitializer);

                        if (!isFunctionExcluded && !funcDef.IsStatic)
                        {
                            ...
                            if (CoreUtils.IsGetterSetter(procName))
                            {

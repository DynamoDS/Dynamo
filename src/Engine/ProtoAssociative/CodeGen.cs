    ...
    public class CodeGen : ProtoCore.CodeGen
    {
        ...
        private bool CyclicDependencyTest(ProtoCore.AssociativeGraph.GraphNode node, ref SymbolNode cyclicSymbol1, ref SymbolNode cyclicSymbol2)
        {
            ...
            else
            {
                ...
                string message = String.Format(ProtoCore.Resource.kInvalidStaticCyclicDependency, symbol1, symbol2);
                                ...
                                ref ProtoCore.Type inferedType)
        {
                ...
                if (toResolveMethodOnClass)
                {
                    ...
                    if (CoreUtils.TryGetPropertyName(procName, out property))
                    {
                        ...
                        if (procCallNode == null)
                        {
                            ...
                            {
                                string message = String.Format(Resource.kCallingNonStaticProperty,
                                                               className,
                                                               property);
                    ...
                    else
                    {
                        ...
                        if (!isStaticCall && !isConstructor)
                        {
                            if (subPass == ProtoCore.Compiler.Associative.SubCompilePass.kNone)
                            {
                                string message = String.Format(Resource.kStaticMethodNotFound,
                                                               className,
                                                               procName);
                ...
                else if (hasAllocated && symbolnode.datatype.UID != (int)PrimitiveType.kTypeVar)
                {
                    ...
                    if (null != procCallNode)
                    {
                        if (procCallNode.isConstructor)
                        {
                            if (subPass == ProtoCore.Compiler.Associative.SubCompilePass.kNone)
                            {
                                string message = String.Format(Resource.KCallingConstructorOnInstance,
                                                               procName);
                        ...
                        if (!isAccessible)
                        {
                            if (subPass == ProtoCore.Compiler.Associative.SubCompilePass.kNone)
                            {
                                string message = String.Format(ProtoCore.Resource.kMethodIsInaccessible, procName);
            ...
            if ((procCallNode == null) && (procName != Constants.kFunctionPointerCall) && (parentNode == null))
            {
                if (globalClassIndex != Constants.kInvalidIndex)
                {
                    ...
                    if (memProcNode != null)
                    {
                        ...
                        if (!isAccessible)
                        {
                            string message = String.Format(Resource.kMethodIsInaccessible, procName);
            ...
            else
            {
                    ...
                    (globalClassIndex == inferedType.UID))
                {
                    ...
                    {
                        string message = String.Format(Resource.kCallingConstructorInConstructor, procName);
            ...
            ProtoCore.AST.Node bnode = null)
        {
            ...
            else
            {
                if (depth <= 0 && procName != ProtoCore.DSASM.Constants.kFunctionPointerCall)
                {
                    ...
                    if (CoreUtils.TryGetPropertyName(procName, out property))
                    {
                        string message = String.Format(ProtoCore.Resource.kPropertyNotFound, property);
                    ...
                    {
                        string message = String.Format(ProtoCore.Resource.kMethodNotFound, procName);
        ...
        */
        ...
        private void EmitIdentifierNode(AssociativeNode node, ref ProtoCore.Type inferedType, bool isBooleanOp = false, ProtoCore.AssociativeGraph.GraphNode graphNode = null, ProtoCore.Compiler.Associative.SubCompilePass subPass = ProtoCore.Compiler.Associative.SubCompilePass.kNone, BinaryExpressionNode parentNode = null)
        {
            ...
            if (t.Name.Equals(ProtoCore.DSDefinitions.Keyword.This))
            {
                ...
                if (localProcedure != null)
                {
                    ...
                    {
                        string message = ProtoCore.Resource.kUsingThisInStaticFunction;
                    ...
                    {
                        string message = ProtoCore.Resource.kInvalidThis;
                        core.BuildStatus.LogWarning(ProtoCore.BuildData.WarningID.kInvalidThis, message, core.CurrentDSFileName, t.line, t.col, graphNode);
                        EmitPushNull();
                        return;
                    }
            ...
            if (ProtoCore.Compiler.Associative.SubCompilePass.kUnboundIdentifier == subPass)
            {
                if (symbolnode == null)
                {
                    ...
                    if (isAllocated)
                    {
                        string message = String.Format(Resource.kPropertyIsInaccessible, t.Value);
                        if (localProcedure != null && localProcedure.isStatic)
                        {
                            ...
                            {
                                message = String.Format(Resource.kUsingNonStaticMemberInStaticContext, t.Value);
                            }
                        }
                    ...
                    {
                        string message = String.Format(Resource.kUnboundIdentifierMsg, t.Value);
                ...
                switch (stepOp)
                {
                    case ProtoCore.DSASM.RangeStepOperator.stepsize:
                        ...
                        if (!hasAmountOperator)
                        {
                            ...
                            {
                                isStepValid = false;
                                warningMsg = Resource.kRangeExpressionWithStepSizeZero;
                            }
                            else if ((end > current && step < 0) || (end < current && step > 0))
                            {
                                isStepValid = false;
                                warningMsg = Resource.kRangeExpressionWithInvalidStepSize;
                            }
                        }
                       ...
                       else
                       {
                               ...
                               subPass == ProtoCore.Compiler.Associative.SubCompilePass.kNone)
                           {
                               buildStatus.LogWarning(WarningID.kInvalidRangeExpression,
                                                      Resource.kRangeExpressionWithNonIntegerStepNumber,
                                                      core.CurrentDSFileName,
                                                      stepNode.line,
                                                      stepNode.col, 
                                                      graphNode);
                           ...
                           {
                               isStepValid = false;
                               warningMsg = Resource.kRangeExpressionWithNegativeStepNumber;
                           }
                       }
                    ...
                    case ProtoCore.DSASM.RangeStepOperator.approxsize:
                        ...
                        {
                            isStepValid = false;
                            warningMsg = Resource.kRangeExpressionConflictOperator;
                        }
                        else if (step == 0)
                        {
                            isStepValid = false;
            ...
            foreach (VarDeclNode vardecl in classDecl.varlist)
            {
                ...
                else if (!classDecl.IsExternLib)
                {
                    ...
                    else
                    {
                        ...
                        {
                            string message = String.Format(ProtoCore.Resource.kTypeUndefined, typeName);
        ...
        private void EmitConstructorDefinitionNode(AssociativeNode node, ref ProtoCore.Type inferedType, ProtoCore.Compiler.Associative.SubCompilePass subPass = ProtoCore.Compiler.Associative.SubCompilePass.kNone, GraphNode gNode = null)
        {
            ...
            if (IsParsingMemberFunctionSig())
            {
                ...
                {
                    string message = String.Format(Resource.kMethodAlreadyDefined, localProcedure.name);
        ...
        private void EmitFunctionDefinitionNode(AssociativeNode node, ref ProtoCore.Type inferedType, ProtoCore.Compiler.Associative.SubCompilePass subPass = ProtoCore.Compiler.Associative.SubCompilePass.kNone, GraphNode graphNode = null)
        {
            ...
            if (IsParsingGlobalFunctionSig() || IsParsingMemberFunctionSig())
            {
                ...
                {
                    string message = String.Format(ProtoCore.Resource.kReturnTypeUndefined, funcDef.ReturnType.Name, funcDef.Name);
                ...
                {
                    string message = String.Format(ProtoCore.Resource.kMethodAlreadyDefined, localProcedure.name);
            ...
            else if (parseGlobalFunctionBody || parseMemberFunctionBody)
            {
                ...
                if (!hasReturnStatement && !funcDef.IsExternLib)
                {
                    ...
                    {
                        string message = String.Format(ProtoCore.Resource.kFunctionNotReturnAtAllCodePaths, localProcedure.name);
        ...
        */
            ...
            */
            ...
            if (ProtoCore.DSASM.Operator.assign == bnode.Optr)
            {
                ...
                else if (bnode.LeftNode is IdentifierNode)
                {
                    ...
                    {
                        string errorMessage = ProtoCore.Resource.kInvalidThis;
                        if (localProcedure != null)
                        {
                            if (localProcedure.isStatic)
                            {
                                errorMessage = ProtoCore.Resource.kUsingThisInStaticFunction;
                            }
                            else if (localProcedure.classScope == Constants.kGlobalScope)
                            {
                                errorMessage = ProtoCore.Resource.kInvalidThis;
                            }
                            else
                            {
                                errorMessage = ProtoCore.Resource.kAssingToThis;
                            }
                        }
            ...
            if (bnode.LeftNode is IdentifierNode)
            {
                ...
                else
                {
                    ...
                    {
                        string message = String.Format(Resource.kPropertyIsInaccessible, t.Name);
        ...
        protected void EmitExceptionHandlingNode(AssociativeNode node, ProtoCore.AssociativeGraph.GraphNode graphNode = null, ProtoCore.Compiler.Associative.SubCompilePass subPass = ProtoCore.Compiler.Associative.SubCompilePass.kNone)
        {
            ...
            foreach (var catchBlock in exceptionNode.catchBlocks)
            {
                ...
                {
                    string message = String.Format(ProtoCore.Resource.kExceptionTypeUndefined, filterNode.type.Name);
        ...
        private ProtoCore.Type BuildArgumentTypeFromVarDeclNode(VarDeclNode argNode, GraphNode graphNode = null)
        {
            ...
            {
                string message = String.Format(Resource.kArgumentTypeUndefined, argNode.ArgumentType.Name, argNode.NameNode.Name);

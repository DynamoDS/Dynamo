...
namespace ProtoAssociative
{
    ...
    public class CodeGen : ProtoCore.CodeGen
    {
        ...
        private void PushSymbolAsDependentToGraphNode(SymbolNode symbol, UpdateNodeType nodeType, ProtoCore.AssociativeGraph.GraphNode graphNode)
        {
            // Check for symbols that need to be push as dependents
            ...
            if (ProtoCore.Utils.CoreUtils.IsPropertyTemp(symbol.name) || ProtoCore.Utils.CoreUtils.IsDefaultArgTemp(symbol.name))
            {
                return;
            }

            ProtoCore.AssociativeGraph.GraphNode dependentNode = new ProtoCore.AssociativeGraph.GraphNode();
            dependentNode.PushSymbolReference(symbol, nodeType);
                                              ...
                                              BinaryExpressionNode bnode)
        {
            ...
            if (subPass != ProtoCore.Compiler.Associative.SubCompilePass.kUnboundIdentifier && graphNode != null)
            {
                if (isStaticCall)
                {
                    ...
                    PushSymbolAsDependentToGraphNode(classSymbol, UpdateNodeType.kSymbol, graphNode);
                }
                                ...
                                ref ProtoCore.Type inferedType)
        {
            ...
            if ((procCallNode == null) && (procName != Constants.kFunctionPointerCall))
            {
                ...
                if (isAllocated) // not checking the type against function pointer, as the type could be var
                {
                    ...
                    if (null != graphNode)
                    {
                        PushSymbolAsDependentToGraphNode(symbolnode, UpdateNodeType.kSymbol, graphNode);
                    }
                }
            }
            ...
            else
            {
                    ...
                    (globalClassIndex == inferedType.UID))
                {
                        ...
                        contextProcNode.runtimeIndex == procCallNode.runtimeIndex)
                    {
                        ...
                        buildStatus.LogWarning(WarningID.kCallingConstructorInConstructor, message, core.CurrentDSFileName, node.line, node.col, guid);
                        inferedType.UID = (int)PrimitiveType.kTypeNull;
                        EmitPushNull();
                        return procCallNode;
                    }
            ...
            if (ProtoCore.Compiler.Associative.SubCompilePass.kUnboundIdentifier == subPass)
            {
                if (symbolnode == null)
                {
                    ...
                    else
                    {
                        ...
                        buildStatus.LogWarning(WarningID.kIdUnboundIdentifier, message, core.CurrentDSFileName, t.line, t.col, guid);
                    }
        ...
        private void EmitConstructorDefinitionNode(AssociativeNode node, ref ProtoCore.Type inferedType, ProtoCore.Compiler.Associative.SubCompilePass subPass = ProtoCore.Compiler.Associative.SubCompilePass.kNone, GraphNode gNode = null)
        {
            ...
            if (IsParsingMemberFunctionSig())
            {
                ...
                else
                {
                    ...
                    buildStatus.LogWarning(ProtoCore.BuildData.WarningID.kFunctionAlreadyDefined, message, core.CurrentDSFileName, funcDef.line, funcDef.col, guid);
                    funcDef.skipMe = true;
                }
            }
            ...
            else if (parseGlobalFunctionBody || parseMemberFunctionBody)
            {
                ...
                if (!hasReturnStatement && !funcDef.IsExternLib)
                {
                    if (!core.Options.SuppressFunctionResolutionWarning)
                    {
                        ...
                        buildStatus.LogWarning(ProtoCore.BuildData.WarningID.kMissingReturnStatement, message, core.CurrentDSFileName, funcDef.line, funcDef.col, guid);
                    }
                    EmitReturnNull();
                }

...
namespace DynamoPythonTests
{
    ...
    internal class SharedCodeCompletionProviderTests : UnitTestBase
    {
        public override void Setup()
        {
            ...
            Assembly.LoadFrom(Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "DSPythonNet3.dll"));
        }

        [Test]
        public void SharedCoreCanFindLoadedProviders()
        {
            var provider = new SharedCompletionProvider(PythonEngineManager.PythonNet3EngineName, "");
    ...
    internal class CodeCompletionTests
    {
        ...
        public void VerifyPythonLoadedAssemblies()
        {
            ...
            {
                if (assembly.FullName.StartsWith("DSPythonNet3"))
                {
                    if (matches.Contains(assembly.FullName))
                    {
                        Assert.Fail("Attempted to load an DSPythonNet3 assembly multiple times: " + assembly.FullName);
                    }
                    else
                    {

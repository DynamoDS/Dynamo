...
namespace Dynamo.Tests
{
    ...
    public class PythonEditTests : DynamoViewModelUnitTest
    {
        ...
        public void TwoCPythonHandlesReturnedFromSameNodeHaveSameHandleID()
        {
            ...
            ViewModel.HomeSpace.Run();
            var handles = classdef.CachedValue.GetElements().Select(x => x.Data).Cast<DynamoPythonNet3Handle>().ToList<DynamoPythonNet3Handle>();
        ...
        public void TwoCPythonHandlesReturnedFromDifferentNodesHaveSameHandleID()
        {
            ...
            var dynamoHandles = handles.Cast<DynamoPythonNet3Handle>().ToList();
        ...
        public void CPythonClassCanBeReturnedAndSafelyDisposedInDownStreamNode()
        {
            ...
            ViewModel.HomeSpace.Run();
            AssertPreviewValue(downstream2.GUID.ToString(), "joe");
            Assert.AreEqual(2, DynamoPythonNet3Handle.HandleCountMap.First(x => x.ToString().Contains("myClass")).Value);
            ...
            Assert.AreEqual(1, DynamoPythonNet3Handle.HandleCountMap.First(x => x.ToString().Contains("myClass")).Value);

            var deleteCmd2 = new DynamoModel.DeleteModelCommand(classdef.GUID);
            ViewModel.Model.ExecuteCommand(deleteCmd2);

            Assert.IsEmpty(DynamoPythonNet3Handle.HandleCountMap.Where(x => x.ToString().Contains("myClass")));
        }
...
@"class reloaded_class:
    ...
    def get_data(self):
        return self.data";
                ...
                //after a second run - the old instance should have been disposed
                //only the new one should remain.
                Assert.AreEqual(1, DynamoPythonNet3Handle.HandleCountMap.First(x => x.ToString().Contains("reloaded_class")).Value);

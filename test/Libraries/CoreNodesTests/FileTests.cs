...
namespace Dynamo.Tests
{
    public class FileLibraryTests : UnitTestBase
    {
        ...
        {
            Assert.AreEqual(Path.Combine("test"), FilePath.CombinePath("test"));
            Assert.AreEqual(Path.Combine("test", "1"), FilePath.CombinePath("test", "1"));
            Assert.AreEqual(Path.Combine("test/", @"1\"), FilePath.CombinePath("test/", @"1\"));

            const string aFilePath = @"hello\there.txt";
            const string aFileName = "hello";

            Assert.AreEqual(Path.GetExtension(aFilePath), FilePath.FileExtension(aFilePath));
            Assert.AreEqual(Path.GetExtension(aFileName), FilePath.FileExtension(aFileName));

            Assert.AreEqual(
                Path.ChangeExtension(aFilePath, ".png"),
                FilePath.ChangePathExtension(aFilePath, ".png"));
            Assert.AreEqual(
                Path.ChangeExtension(aFileName, ".txt"),
                FilePath.ChangePathExtension(aFileName, ".txt"));
            ...
            Assert.AreEqual(Path.HasExtension(aFilePath), FilePath.FileHasExtension(aFilePath));
            Assert.AreEqual(Path.HasExtension(aFileName), FilePath.FileHasExtension(aFileName));
        }
        ...
        public void Directory_Move()
        {
            ...
            File.WriteText(FilePath.CombinePath(tmpSrc, fileName), "test");
            ...
            var destFileName = FilePath.CombinePath(tmpDest, fileName);
        ...
        public void Directory_Contents()
        {
            ...
            var tmpSrcInfo = DSCore.IO.Directory.FromPath(tmpSrc);
            const string fileName = @"temp.txt";
            var newFile = FilePath.CombinePath(tmpSrc, fileName);
            File.WriteText(newFile, "test");

            const string dirName = @"subDir";
            var newDir = FilePath.CombinePath(tmpSrc, dirName);

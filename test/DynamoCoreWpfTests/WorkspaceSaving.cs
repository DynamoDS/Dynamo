...
namespace Dynamo.Tests
{
    public class WorkspaceSaving : DynamoViewModelUnitTest
    {
        ...
        public void CustomNodeWorkspaceSavedToSamePlaceNotCausingDuplicatedLibraryItems()
        {
            ...
            var res = def.SaveAs(tempPath1);
            Assert.IsTrue(res);
            Thread.Sleep(1);

            res = def.SaveAs(tempPath2);
            Assert.IsTrue(res);
            Thread.Sleep(1);

            res = def.SaveAs(tempPath1);
            Assert.IsTrue(res);

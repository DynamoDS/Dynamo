...
// LC: Removal of cusotm folder generation which was not stable when used via a plugin load
    ...
    internal static class AppDataContractSerializer
    {
        ...
        public static T Restore<T>(string filename = null, bool deleteBadData = false)
        {
            ...
            try
            {
                ...
                try
                {
                    using (FileStream inputStream = new FileStream(file, FileMode.Open, FileAccess.Read, FileShare.Read, 4096, true))
                    {
        ...
        public static void Save<T>(T value, string filename = null)
        {
            ...
            using (var memoryStream = new MemoryStream())
            {
                ...
                using (FileStream fileStream = new FileStream(file, FileMode.Create, FileAccess.Write, FileShare.None, 4096, true))
                {
                    memoryStream.Seek(0, SeekOrigin.Begin);
                    //LC: await removed

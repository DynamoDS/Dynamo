...
// LC: Modified to work in .NET 4.0, removal of async/await
...
namespace CSharpAnalytics
{
    ...
    public abstract class BaseAutoMeasurement
    {
        ...
        public void Start(MeasurementConfiguration configuration, string launchKind, TimeSpan? uploadInterval = null)
        {
            if (!isStarted)
            {
                isStarted = true;
                ...
                var sessionState = Load<SessionState>(SessionStorageName);
        ...
        protected abstract T Load<T>(string name);
        ...
        protected abstract void Save<T>(T data, string name);
        ...
        protected void StopRequester()
        {
            ...
            {
                var pendingRequests = safeBackgroundRequester.Stop();

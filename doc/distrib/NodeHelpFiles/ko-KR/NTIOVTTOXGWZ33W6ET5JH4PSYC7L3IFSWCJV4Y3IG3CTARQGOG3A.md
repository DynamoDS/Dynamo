...
## 상세
`TSplineSurface.BridgeEdgesToEdges`는 동일한 표면 또는 두 개의 서로 다른 표면의 두 개의 모서리 세트를 연결합니다. 노드에는 아래에 설명된 입력이 필요합니다. 교량을 생성하기 위해서는 처음 3개의 입력이면 충분하고 나머지 입력은 선택 사항입니다. 결과 표면은 첫 번째 모서리 그룹이 속한 표면의 하위 항목입니다.
...
아래 예에서는 두 개의 T-Spline 평면이 작성되고 각각의 중심에 있는 면이 `TSplineSurface.DeleteEdges` 노드를 사용하여 삭제됩니다. 삭제된 면 주위의 모서리는 `TSplineTopology.VertexByIndex` 노드를 사용하여 수집됩니다. 교량을 작성하기 위해 2개의 모서리 그룹이 표면 중 하나와 함께 `TSplineSurface.BrideEdgesToEdges` 노드의 입력으로 사용됩니다. 이를 통해 교량이 작성됩니다. `spansCounts` 입력을 편집하면 교량에 더 많은 스팬이 추가됩니다. 곡선이 `followCurves`의 입력으로 사용되면 교량은 제공된 곡선의 방향을 따릅니다. `keepSubdCreases`,`frameRotations`, `firstAlignVertices` 및 `secondAlignVertices` 입력은 교량의 모양을 미세 조정할 수 있는 방법을 보여줍니다.

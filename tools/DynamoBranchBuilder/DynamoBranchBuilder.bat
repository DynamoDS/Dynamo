...
REM Build Dynamo
%msbuild% .\src\Dynamo.All.2012.sln /t:clean,build /p:platform="Any CPU" /p:Configuration=Release
...
call %repo%\tools\install\CreateInstallers-RELEASE.bat

REM Rename the installer to include the branch name
chdir %installersDir%
rename InstallDynamo*.exe DynamoInstaller_%branchName%.exe

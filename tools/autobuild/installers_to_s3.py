	...
	try:
		...
		for file in os.listdir('temp'):
		  if fnmatch.fnmatch( file, '*.exe'):
		  	dynamo_s3.upload_installer( form_path( ['temp', file] ), prefix, include_date, is_dev_build ,'.exe')
		  elif fnmatch.fnmatch( file, '*.sig'):
		  	dynamo_s3.upload_installer( form_path( ['temp', file] ), prefix, include_date, is_dev_build ,'.sig')
